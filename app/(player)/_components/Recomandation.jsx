...
export default function Recomandation({ id }) {
  const genres = ['lekompo', 'amapiano', 'hiphop', 'american-hiphop'];
  const [data, setData] = useState([]);
  const [loading, setLoading] = useState(true);
  const next = useContext(NextContext);

  const getData = async () => {
    const promises = genres.map(genre => fetch(`${process.env.NEXT_PUBLIC_API_URL}songs/${genre}/suggestions/${id}`));
    await Promise.all(promises)
      .then(responses => Promise.all(responses.map(response => response.json())))
      .then(data => {
        const allSongs = data.flat();
        setData(allSongs);
        let d = allSongs[Math.floor(Math.random() * allSongs.length)];
